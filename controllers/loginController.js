...
const getLogin = async (req, res) =>{
    ...
    let query = `SELECT EM.EmployeeId, EM.EmployeeName, U.EmailId, DM.DepartmentName, RM.RoleName FROM userpool AS U JOIN employeemaster AS EM ON EM.Id = U.refEmployeeMaster LEFT JOIN departmentmaster AS DM ON DM.Id = EM.refDepartment JOIN rolemaster AS RM ON RM.Id = EM.refRole WHERE U.EmailId = ? AND U.password =?;`;
